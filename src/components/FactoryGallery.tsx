    ...
    >
      <div className="gallery-stage">
        {images.map((image, index) => (
          <figure
            className={`gallery-slide${index === active ? ' active' : ''}`}
            key={image}
        ...
        {total > 1 ? (
          <>
            <button className="gallery-arrow prev" onClick={() => goTo(active - 1)} aria-label="上一张照片">
              <ChevronLeft aria-hidden="true" size={22} />
            </button>
            <button className="gallery-arrow next" onClick={() => goTo(active + 1)} aria-label="下一张照片">
        ...
        <div className="gallery-thumbs" role="tablist" aria-label="选择照片">
          {images.map((image, index) => (
            <button
              ...
              onClick={() => goTo(index)}
            >
              <Image src={image} alt="" fill sizes="160px" loading="lazy" />
