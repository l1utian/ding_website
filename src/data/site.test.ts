...
describe('site data', () => {
  ...
  it('keeps the two company product catalogs isolated', () => {
    expect(getSite('jike').products).toHaveLength(7)
    ...
    expect(getProductBySlug(getSite('jike'), 'aos-508')).toBeDefined()
  ...
  it('uses Word product documents for product detail content', () => {
    ...
    const aos508 = getProductBySlug(getSite('jike'), 'aos-508')
    ...
    expect(getDocumentPlainText(aos508!.document)).toContain('AOS-508 压延有机环保稳定剂 技术数据表（TDS）')
    expect(getDocumentPlainText(aos508!.document)).toContain('硬质 PVC 压延膜专用复合型环保热稳定剂')
