...
readonly REMOTE_SCRIPT="/tmp/ding_website_deploy_production.sh"
readonly LOCAL_ARCHIVE="${TMPDIR:-/tmp}/ding_website_dist.tar.gz"
readonly LOCAL_REMOTE_SCRIPT="${TMPDIR:-/tmp}/ding_website_deploy_production.sh"
...
ask_first_deploy() {
  ...
  read -r -p "是否首次部署生产环境？首次会写入正式域名 Nginx 配置，默认 no [y/N]: " answer
  case "$answer" in
...
readonly PROD_CONF="/etc/nginx/conf.d/ding_website.conf"
...
disable_test_config() {
  ...
  mv "$TEST_CONF" "${TEST_CONF}.bak.${TIMESTAMP}"
}
...
configure_nginx_if_needed() {
  ...
  log "Production Nginx config enabled"
}
...
main() {
  ...
  log_success "Jike:  http://jike.tech.gd.cn"
  log_success "Alpha: http://alpha.tech.gd.cn"
}
